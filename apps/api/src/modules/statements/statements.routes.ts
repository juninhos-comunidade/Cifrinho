...
export async function statementRoutes(app: FastifyInstance) {
  ...
  app.post("/statements/parse", { ...authenticate }, async (request, reply) => {
    ...
    if (buffer.length > 5 * 1024 * 1024) {
      return reply
        .status(400)
        .send({ message: "Arquivo muito grande. Limite: 5 MB." });
    }
    ...
    if (mimetype === "application/pdf" || filename.endsWith(".pdf")) {
      ...
      return reply
        .status(400)
        .send({ message: "Formato não suportado. Envie PDF, CSV ou OFX." });
    }
    ...
    if (!Array.isArray(transactions) || transactions.length === 0) {
      return reply
        .status(422)
        .send({ message: "Nenhuma transação encontrada no extrato." });
    }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!Array.isArray(transactions) || transactions.length === 0) {
        return reply
          .status(400)
          .send({ message: "Nenhuma transação para importar." });
      }
      ...
      const created = await prisma.$transaction(
        transactions.map((t) => {
          const resolvedCategoryId =
            t.categoryId ??
            (t.suggestedCategory
              ? (upsertedMap[t.suggestedCategory] ?? null)
              : null);
      ...
      return reply
        .status(201)
        .send({ imported: created.length, transactions: created });
    },
  );
}
