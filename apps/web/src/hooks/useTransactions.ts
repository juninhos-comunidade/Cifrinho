...
export function calcOverview(txs: Transaction[]) {
  ...
  const savingsRate =
    curIncome > 0
      ? Math.round(((curIncome - curExpense) / curIncome) * 100)
      : 0;
...
export function calcPersonal(txs: Transaction[]) {
  ...
  for (const t of personal.filter(
...
export function calcBusiness(txs: Transaction[]) {
  ...
  const yearIncome = biz
    .filter((t) => {
      ...
      return (
        t.type === "INCOME" && d.getFullYear() === new Date().getFullYear()
      );
    })
    .reduce((s, t) => s + Number(t.amount), 0);
...
export function calcCategoryPie(
  ...
  const filtered = txs.filter((t) => {
    const d = new Date(t.date);
    return (
      t.type === type && d.getFullYear() === year && d.getMonth() === month
    );
  });
...
export function calcCashFlowBars(
  txs: Transaction[],
  accountType: "PERSONAL" | "BUSINESS",
) {
  const now = new Date();
