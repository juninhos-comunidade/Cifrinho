...
export default function BusinessPage() {
  ...
  const { balance: bizBalance, curIncome: bizIncome, curExpense: bizExpense, meiPct, yearIncome } = calcBusiness(txs)
  const pfBalance  = txs.filter(t => t.accountType === 'PERSONAL').reduce((s, t) => t.type === 'INCOME' ? s + Number(t.amount) : s - Number(t.amount), 0)
  const pfIncome   = txs.filter(t => t.accountType === 'PERSONAL' && t.type === 'INCOME'  && isCurrentMonth(t.date)).reduce((s, t) => s + Number(t.amount), 0)
  const pfExpense  = txs.filter(t => t.accountType === 'PERSONAL' && t.type === 'EXPENSE' && isCurrentMonth(t.date)).reduce((s, t) => s + Number(t.amount), 0)
          ...
          <div>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                ...
                <p className="mt-2 text-3xl font-extrabold tracking-tight text-ink">{fmt(pfBalance)}</p>
                <p className="mt-1.5 text-xs text-mute">Conta pessoal</p>
              </div>
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                <p className="text-sm text-mute">Entradas (mês)</p>
                <p className="mt-2 text-3xl font-extrabold tracking-tight text-brand">{fmt(pfIncome)}</p>
                <p className="mt-1.5 text-xs text-mute">
                  {txs.filter(t => t.accountType === 'PERSONAL' && t.type === 'INCOME' && isCurrentMonth(t.date)).length} lançamentos
                </p>
              </div>
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                <p className="text-sm text-mute">Saídas (mês)</p>
                <p className="mt-2 text-3xl font-extrabold tracking-tight text-ink">{fmt(pfExpense)}</p>
                <p className="mt-1.5 text-xs text-mute">
                  {txs.filter(t => t.accountType === 'PERSONAL' && t.type === 'EXPENSE' && isCurrentMonth(t.date)).length} lançamentos
                </p>
              </div>
            </div>
                ...
                <div className="mt-6 flex h-40 items-end justify-between gap-4">
                  {pfBars.map((b) => (
                    <div key={b.m} className="flex flex-1 flex-col items-center gap-2">
                      <div className={`bar w-full max-w-[40px] ${b.net >= 0 ? (b.cur ? 'bg-brand' : 'bg-brand/70') : 'bg-rose/70'}`} style={{ height: b.h }}></div>
          ...
          <div>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                ...
                <p className="mt-2 text-3xl font-extrabold tracking-tight text-ink">{fmt(bizBalance)}</p>
                <p className="mt-1.5 text-xs text-mute">Conta empresarial</p>
              </div>
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                <p className="text-sm text-mute">Faturamento (mês)</p>
                <p className="mt-2 text-3xl font-extrabold tracking-tight text-brand">{fmt(bizIncome)}</p>
                <p className="mt-1.5 text-xs text-mute">
                  {txs.filter(t => t.accountType === 'BUSINESS' && t.type === 'INCOME' && isCurrentMonth(t.date)).length} lançamentos
                </p>
              </div>
              <div className="rounded-lg border border-line bg-card p-5 elev-sm">
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-bg">
                  <div className={`h-full rounded-full ${meiPct > 80 ? 'bg-rose' : meiPct > 60 ? 'bg-amber' : 'bg-brand'}`} style={{ width: `${meiPct}%` }}></div>
                </div>
            ...
            <div className="mt-5 rounded-lg border border-line bg-card p-6 elev-sm">
              ...
              {pjBars.every(b => b.net === 0) ? (
                ...
                <div className="mt-6 flex h-40 items-end justify-between gap-4">
                  {pjBars.map((b) => (
                    <div key={b.m} className="flex flex-1 flex-col items-center gap-2">
                      <div className={`bar w-full max-w-[40px] ${b.net >= 0 ? (b.cur ? 'bg-blue' : 'bg-blue/70') : 'bg-rose/70'}`} style={{ height: b.h }}></div>
                      <span className={`text-[11px] ${b.cur ? 'font-semibold text-ink' : 'text-faint'}`}>{b.m}</span>
                    </div>
                  ))}
                </div>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-mute">
                            {t.category?.name ?? 'Sem categoria'} · {new Date(t.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
                          </p>
                        </div>
