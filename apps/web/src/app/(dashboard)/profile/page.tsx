...
export default function ProfilePage() {
  ...
  const joinedYear = user
    ...
    : null
  const joinedLabel = joinedYear === 0 ? 'este ano' : joinedYear === 1 ? '1 ano' : `${joinedYear} anos`
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {isLoading ? (
              ...
              [
                ...
                { label: 'Membro desde',  value: user ? new Date(user.createdAt).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }) : '—' },
                { label: 'ID da conta',   value: user ? `#${user.id.slice(0, 8).toUpperCase()}` : '—' },
              ].map((f, i) => (
                <div key={i}>
