...
const COPY: Record<ServiceStatus, { badge: string; title: ReactNode; sub: string; uptime: string; services: string }> = {
  operational: {
    badge:    'Todos os sistemas operacionais',
    title:    <>Tudo certo<br />por aqui.</>,
  ...
  maintenance: {
    ...
    title:    <>Em manutenção<br />programada.</>,
...
const SERVICE_ICONS: Record<string, ReactNode> = {
  ...
  reports: (
    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 3v16a2 2 0 0 0 2 2h16"/><rect x="7" y="11" width="3" height="6" rx="1"/><rect x="13" y="7" width="3" height="10" rx="1"/>
    </svg>
  ),
}
...
const SEVERITY_LABELS: Record<Severity, string> = { low: 'BAIXA', medium: 'MÉDIA', high: 'ALTA' }
const SEVERITY_VAR:   Record<Severity, string>  = { low: 'var(--c-blue)', medium: 'var(--c-amber)', high: 'var(--c-rose)' }
...
const FALLBACK: StatusPayload = {
  ...
  services: [
    { name: 'API',             key: 'api',           status: 'operational', latency_ms: 0,  uptime_pct: '99,99%' },
    { name: 'Banco de Dados',  key: 'database',      status: 'operational', latency_ms: 0,  uptime_pct: '99,97%' },
    { name: 'Autenticação',    key: 'auth',          status: 'operational', latency_ms: 0,  uptime_pct: '99,82%' },
    { name: 'Notificações',    key: 'notifications', status: 'operational', latency_ms: 0,  uptime_pct: '99,90%' },
    { name: 'Relatórios & IR', key: 'reports',       status: 'operational', latency_ms: 0,  uptime_pct: '99,90%' },
  ],
  incidents: [
    { id: '1', title: 'Manutenção programada · Autenticação', description: 'Janela de atualização do provedor de login.', severity: 'medium', date: '02/06/2026', resolved: true },
    { id: '2', title: 'Lentidão em relatórios',               description: 'Exportações de IR lentas por 22 min. Resolvido.', severity: 'high',   date: '28/05/2026', resolved: true },
    { id: '3', title: 'Ajuste em webhooks de PIX',            description: 'Pequeno atraso na confirmação de pagamentos.',    severity: 'low',    date: '15/05/2026', resolved: true },
  ],
...
export default async function StatusPage() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden font-sans" style={{ backgroundColor: 'rgb(var(--c-bg-deep))' }}>
      <div
        className="absolute inset-0 overflow-hidden rounded-2xl border"
        style={{ backgroundColor: 'rgb(var(--c-bg))', borderColor: 'rgb(var(--c-line))' }}
      >

        {/* TOP BAR */}
        <div className="flex items-center justify-between border-b px-8 py-5" style={{ borderColor: 'rgb(var(--c-line))' }}>
          <a href="/" className="flex items-center gap-2.5">
            ...
            <span
              ...
              style={{ borderColor: 'rgb(var(--c-line))', background: 'rgb(var(--c-card))', color: 'rgb(var(--c-mute))' }}
            >
              Status
            </span>
          </a>
        ...
        <div className="grid h-[calc(100%-73px)] grid-cols-[3fr_4fr_3fr] gap-6 p-6">
          ...
          >
            ...
            <div
              ...
              style={{ borderColor: 'rgb(var(--c-line))', background: 'rgb(var(--c-bg))' }}
            >
              <span className="relative flex h-2.5 w-2.5">
                <span className="absolute inline-flex h-full w-full animate-ping rounded-full opacity-70" style={{ background: `rgb(${acc})` }} />
                <span className="relative inline-flex h-2.5 w-2.5 rounded-full" style={{ background: `rgb(${acc})` }} />
              </span>
              <span className="text-sm font-bold" style={{ color: `rgb(${acc})` }}>{copy.badge}</span>
            </div>
            ...
            <div className="mt-6 grid w-full max-w-xs grid-cols-2 gap-3">
              <div className="rounded-xl border p-3" style={{ borderColor: 'rgb(var(--c-line))', background: 'rgb(var(--c-bg))' }}>
                <p className="text-2xl font-extrabold" style={{ color: `rgb(${acc})` }}>{uptimePct}</p>
          ...
          <div
            ...
            style={{ borderColor: 'rgb(var(--c-line))', background: 'rgb(var(--c-card))' }}
          >
            <p className="text-xs font-semibold uppercase tracking-widest text-mute">Serviços</p>
            <div className="mt-4 flex flex-1 flex-col justify-between gap-3">
              {data.services.map((svc) => {
                const svcAcc  = svc.status === 'maintenance' ? 'var(--c-amber)' : svc.status === 'offline' ? 'var(--c-rose)' : 'var(--c-brand)'
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: `rgb(${svcAcc} / 0.12)`, color: `rgb(${svcAcc})` }}
                    >
                      {icon}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-ink">{svc.name}</p>
                      <p className="text-xs text-mute">{svc.latency_ms > 0 ? `${svc.latency_ms} ms` : 'sem latência'}</p>
                    </div>
                    <span className="text-xs font-semibold text-mute">{svc.uptime_pct}</span>
                    <span
                      className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold"
                      style={{ background: `rgb(${svcAcc} / 0.10)`, color: `rgb(${svcAcc})` }}
                    >
                      <span className="h-1.5 w-1.5 rounded-full" style={{ background: `rgb(${svcAcc})` }} />
          ...
          <div className="flex flex-col">
            <div
              ...
              style={{ borderColor: 'rgb(var(--c-line))', background: 'rgb(var(--c-card))' }}
            >
              <p className="text-xs font-semibold uppercase tracking-widest text-mute">Incidentes recentes</p>
              <div className="mt-4 flex flex-1 flex-col justify-between gap-3">
                {data.incidents.slice(0, 3).map((inc) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px]" style={{ color: 'rgb(var(--c-faint))' }}>{inc.date}</span>
                    </div>
              ...
              <div className="mt-5 border-t pt-5" style={{ borderColor: 'rgb(var(--c-line))' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-widest text-mute">Uptime · 90 dias</p>
                  ...
                  {data.uptime_bars.map((t, i) => (
                    <div
                      ...
                      style={{
                        height: '100%',
                        background:
                          t === 'dn' ? 'rgb(var(--c-rose)  / 0.85)' :
                          t === 'mt' ? 'rgb(var(--c-amber) / 0.85)' :
                                       'rgb(var(--c-brand) / 0.85)',
                      }}
                    />
                  ))}
                </div>
                <div className="mt-3 flex items-center justify-between text-[11px]" style={{ color: 'rgb(var(--c-faint))' }}>
