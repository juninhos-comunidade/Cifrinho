      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        ...
        <div className="space-y-5">
          <div className="rounded-lg border border-line bg-card p-6 elev-sm">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-4xl font-extrabold tracking-tight text-ink">{fmt(balance)}</p>
              </div>
              <button
                onClick={openNew}
          ...
          <div className="rounded-lg border border-line bg-card p-6 elev-sm">
            ...
            <p className="text-xs text-mute">{monthName} · {fmt(curExpense)} no total</p>
            {categories.length === 0 ? (
                  ...
                  <div key={c.name}>
                    <div className="mb-1.5 flex items-center justify-between text-sm text-ink">
                      <span className="flex items-center gap-2">
                        <span className={`h-2.5 w-2.5 rounded-sm ${CAT_COLORS[i % CAT_COLORS.length]}`}></span>
                        {c.name}
                      </span>
                      <span className="font-semibold">{fmt(c.total)} · {c.pct}%</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full bg-bg">
                      <div className={`h-full rounded-full ${CAT_COLORS[i % CAT_COLORS.length]}`} style={{ width: `${c.pct}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="rounded-lg border border-line bg-card elev-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-6 py-4">
            ...
            <div className="seg">
              ...
              <button className={filter === 'income'  ? 'on' : ''} onClick={() => setFilter('income')}>Entradas</button>
              <button className={filter === 'expense' ? 'on' : ''} onClick={() => setFilter('expense')}>Saídas</button>
            </div>
          </div>
              ...
              {filtered.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    {t.category && (
                      <span className="rounded-full bg-elev px-2 py-0.5 text-[11px] font-semibold text-mute">{t.category.name}</span>
                    )}
                    <p className={`w-28 text-right text-sm font-bold ${isIncome ? 'text-brand' : 'text-ink'}`}>
