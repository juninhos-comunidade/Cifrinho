...
function NewBadgeToast({ badge, onDismiss }: { badge: { name: string; description: string }; onDismiss: () => void }) {
  useEffect(() => {
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-mute">
                      {progress?.xp ?? 0} XP · {earned.length} badge{earned.length !== 1 ? 's' : ''} conquistada{earned.length !== 1 ? 's' : ''}
                    </p>
                    ...
                    <p className="mt-1 text-xs text-faint">
                      {progress?.currentXp ?? 0} / {progress?.nextLevelXp ?? 100} XP para o próximo nível
                    </p>
                  </>
                )}
              </div>
          ...
          <div className="rounded-lg border border-line bg-card p-6 elev-sm">
            ...
            {isLoading ? (
              ...
              <div className="mt-5 grid grid-cols-3 gap-4 sm:grid-cols-4">
                ...
                {locked.map((b) => (
                  <div key={b.id} title={b.description} className="rounded-lg border border-dashed border-line bg-bg/50 p-4 text-center opacity-55 cursor-help">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-lg border border-line bg-card p-5 text-center elev-sm">
              {isLoading ? <Skeleton className="mx-auto h-8 w-16 mb-1" /> : (
            ...
            <div className="mt-4 space-y-2">
              {leaderboard.map((p) => (
                ...
                >
                  ...
                  <p className={`flex-1 text-sm ${'me' in p && p.me ? 'font-bold text-ink' : 'font-medium text-ink'}`}>{p.name}</p>
                  <span className={`text-xs font-semibold ${'me' in p && p.me ? 'text-brand font-bold' : 'text-mute'}`}>{p.xp}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-lg border border-line bg-card p-5 elev-sm">
            ...
            <ul className="space-y-2 text-xs text-mute">
              <li className="flex items-center gap-2"><span className="text-brand font-bold">+10</span> por transação lançada</li>
              <li className="flex items-center gap-2"><span className="text-amber font-bold">+100</span> por badge desbloqueada</li>
              <li className="flex items-center gap-2"><span className="text-purple font-bold">Bônus</span> sequência de dias ativos</li>
            </ul>
          </div>
        </div>
      </div>
