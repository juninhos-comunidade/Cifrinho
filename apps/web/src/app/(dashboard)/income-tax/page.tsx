...
export default function IncomeTaxPage() {
  ...
  const pct = data
    ? Math.min(Math.round((data.transactionCount / Math.max(data.transactionCount + 3, 1)) * 100), 100)
    : 0
    ...
    <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
      <div className="space-y-5">
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-line bg-bg px-3 py-1 text-xs font-semibold text-brand">
                ...
                {isLoading ? 'Carregando…' : `Exercício ${(data?.year ?? new Date().getFullYear()) + 1} · ano-base ${data?.year ?? new Date().getFullYear()}`}
              </span>
              {isLoading ? (
              ...
              <p className="mt-1 text-sm text-mute">O Cifrinho organiza seus dados ao longo do ano.</p>
            </div>
            <div className="relative h-28 w-28 shrink-0">
              <svg viewBox="0 0 36 36" className="h-28 w-28 -rotate-90">
                <circle cx="18" cy="18" r="15.5" fill="none" stroke="rgb(var(--c-line))" strokeWidth="4"/>
                <circle cx="18" cy="18" r="15.5" fill="none" stroke="rgb(var(--c-brand))" strokeWidth="4"
        ...
        <div className="rounded-lg border border-line bg-card p-6 elev-sm">
          ...
          {isLoading ? (
            ...
            <div className="mt-4 space-y-3">
              {[
                { label: 'Total de receitas',  value: fmt(data.totalIncome),   color: 'text-brand' },
                { label: 'Total de despesas',  value: fmt(data.totalExpenses), color: 'text-ink' },
                { label: 'Saldo do período',   value: fmt(data.balance),       color: data.balance >= 0 ? 'text-brand' : 'text-rose' },
              ].map((row, i) => (
                <div key={i} className="flex items-center justify-between rounded-md border border-line bg-bg px-4 py-3">
        ...
        <div className="rounded-lg border border-line bg-card p-6 elev-sm">
          ...
          <div className="mt-4 space-y-2.5">
            {[
              { label: 'Informes de rendimento (PF)', status: 'Não iniciado', done: false, warn: false },
              { label: 'Notas fiscais emitidas (MEI)', status: 'Não iniciado', done: false, warn: false },
              { label: 'Recibos médicos dedutíveis', status: 'Não iniciado', done: false, warn: false },
              ...
              <div key={i} className="flex items-center gap-3 rounded-md border border-line bg-bg px-4 py-3">
                {item.done ? (
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-brand text-white">
                    <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6 9 17l-5-5"/></svg>
                  </span>
                ) : item.warn ? (
                  <span className="grid h-6 w-6 place-items-center rounded-full border-2 border-amber text-amber">
                    <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M12 6v6l4 2"/></svg>
                  </span>
                ) : (
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-lg border border-line bg-card p-6 elev-sm">
          ...
          <p className="mt-1.5 text-xs text-mute">Estimativa com base nos lançamentos registrados</p>
          {data && (
            <div className="mt-5 space-y-3 border-t border-line pt-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-semibold ${data.balance >= 0 ? 'text-brand' : 'text-rose'}`}>{fmt(data.balance)}</span>
              </div>
            </div>
          )}
        </div>
