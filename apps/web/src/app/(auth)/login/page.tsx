...
function Mascot({
  ...
  useEffect(() => {
    if (termsAccepted) {
      ...
      setTimeout(() => {
        setShowOink(true)
        setShowHearts(true)
        setTimeout(() => { setShowOink(false); setShowHearts(false) }, 1500)
      }, 16)
    }
  }, [termsAccepted])
  ...
  return (
    <div
      ref={stageRef}
      style={{ position: 'relative', width: 248, height: 248, animation: 'pigfloat 6s ease-in-out infinite' }}
    >
      <div ref={wrapRef} style={{ position: 'absolute', inset: 0, willChange: 'transform', transition: 'transform .28s cubic-bezier(.2,.7,.2,1)' }}>
        <Image
          src="/cifrinho-mascot.png"
          alt="Mascote Cifrinho"
          width={248}
          height={248}
          style={{ objectFit: 'contain', display: 'block', filter: 'drop-shadow(0 20px 32px rgb(33 194 94 / .28))' }}
          priority
        />
        {/* Pálpebras */}
        <span style={{
          ...
          transition: 'transform .16s ease', zIndex: 3,
          left: '25.5%', top: '23.8%', width: '6.8%', height: '10.4%',
        }} />
        <span style={{
          position: 'absolute', background: '#10b981',
          borderRadius: '0 0 65% 65% / 0 0 85% 85%',
          ...
          transformOrigin: 'top center',
          transition: 'transform .16s ease', zIndex: 3,
          left: '68.8%', top: '23.8%', width: '6.8%', height: '10.4%',
        }} />
      </div>

      {/* Relógio de "remember me" */}
      <div style={{
        position: 'absolute', top: -4, left: 18, width: 48, height: 48, zIndex: 5,
        opacity: showClock ? 1 : 0,
        transform: showClock ? 'scale(1) translateY(0) rotate(0)' : 'scale(.3) translateY(10px) rotate(-25deg)',
        transition: 'opacity .25s ease, transform .4s cubic-bezier(.2,1.4,.4,1)',
        pointerEvents: 'none',
      }}>
        <div style={{
          position: 'relative', width: '100%', height: '100%', borderRadius: '50%',
          background: '#fff', border: '3px solid #10b981', boxShadow: '0 8px 18px rgb(0 0 0 / .35)',
        }}>
          <span style={{
            position: 'absolute', left: '50%', bottom: '50%',
            transformOrigin: 'bottom center', background: '#0f172a', borderRadius: 99,
            width: 3.5, height: 10, transform: 'translateX(-50%) rotate(20deg)',
            animation: showClock ? 'rcspin 3.6s linear infinite' : 'none',
          }} />
          <span style={{
            position: 'absolute', left: '50%', bottom: '50%',
            transformOrigin: 'bottom center', background: '#0f172a', borderRadius: 99,
            width: 3, height: 15, transform: 'translateX(-50%) rotate(150deg)',
      ...
      {/* bolha de "oink" */}
      <div style={{
        position: 'absolute', top: '4%', right: -10, zIndex: 6, pointerEvents: 'none',
        background: '#fff', color: '#0f172a', fontWeight: 800, fontSize: 13,
        padding: '5px 11px', borderRadius: '13px 13px 13px 4px', boxShadow: '0 8px 18px rgb(0 0 0 / .32)',
      ...
      {(['left', 'right'] as const).map((side) => (
        <span key={side} style={{
          position: 'absolute',
          width: 22, height: 22, zIndex: 5, pointerEvents: 'none',
          color: '#fb7185',
          ...
          <svg viewBox="0 0 24 24" fill="currentColor" style={{ width: '100%', height: '100%', display: 'block', filter: 'drop-shadow(0 4px 8px rgb(0 0 0 / .28))' }}>
...
export default function LoginPage() {
  ...
  const subtext = tab === 'login' ? 'Entre para acompanhar suas finanças.' : 'Comece de graça em menos de um minuto.'
    ...
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <aside className="relative hidden w-[44%] xl:w-[48%] lg:flex flex-col justify-between overflow-hidden border-r border-line bg-card p-12">
          {/* glow de fundo */}
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 0%, rgb(var(--c-brand) / 0.14), transparent 60%)' }} />
          <div style={{
          ...
          {/* logo */}
          <a href="/" style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: 'inherit' }}>
            <Image src="/cifrinho-mascot.png" alt="Cifrinho" width={40} height={40} />
            <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em' }}>Cifrinho</span>
          </a>

          {/* pitch central com mascote */}
          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
            <div style={{ marginBottom: 32 }}>
              <Mascot pwFocused={pwFocused} rememberOn={rememberFired} termsAccepted={termsAcceptedFired} />
            </div>
            <h2 style={{ maxWidth: 400, fontSize: 36, fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.02em', margin: 0 }}>
              Sua vida financeira,<br />
              <span style={{ color: 'rgb(var(--c-brand))' }}>pessoal e empresarial</span>,<br />
              num só lugar.
            </h2>
            <p style={{ marginTop: 20, maxWidth: 400, fontSize: 18, lineHeight: 1.6, color: 'rgb(var(--c-mute))' }}>
              Organize gastos, separe PF de PJ e deixe seu Imposto de Renda pronto o ano inteiro — com a clareza de quem leva a grana a sério.
            </p>
            ...
            <div style={{ marginTop: 32, display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
              {[
                ...
                <span key={label} style={{
                  ...
                  borderRadius: 99, border: '1px solid rgb(var(--c-line))',
                  background: 'rgb(var(--c-bg) / .6)',
                  padding: '6px 14px', fontSize: 14, fontWeight: 500, color: 'rgb(var(--c-mute))',
                }}>
                  <span style={{
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ display: 'flex' }}>
              {[
                { initials: 'RA', from: 'from-brand', to: 'to-blue', grad: 'linear-gradient(135deg, rgb(var(--c-brand)), rgb(var(--c-blue)))' },
                { initials: 'JV', from: 'from-purple', to: 'to-blue', grad: 'linear-gradient(135deg, rgb(var(--c-purple)), rgb(var(--c-blue)))' },
                { initials: 'MA', from: 'from-amber', to: 'to-rose', grad: 'linear-gradient(135deg, rgb(var(--c-amber)), rgb(var(--c-rose)))' },
                { initials: '+9k', grad: 'rgb(var(--c-elev))', color: 'rgb(var(--c-mute))' },
              ].map(({ initials, grad, color }, i) => (
                <span key={i} style={{
                  display: 'grid', placeItems: 'center',
                  width: 36, height: 36, borderRadius: '50%',
                  background: grad, color: color ?? '#fff',
            ...
            <p style={{ fontSize: 14, color: 'rgb(var(--c-mute))', margin: 0 }}>
              Mais de <span style={{ fontWeight: 600, color: 'rgb(var(--c-ink))' }}>9.000 Juninhos</span> já organizam suas finanças.
            </p>
          </div>
        </aside>

        {/* DIREITA: Painel do formulário */}
        <main style={{ position: 'relative', display: 'flex', flex: 1, flexDirection: 'column', padding: '32px 20px' }}>
          {/* topo: logo mobile */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: 'inherit' }} className="lg:invisible">
              <Image src="/cifrinho-mascot.png" alt="Cifrinho" width={36} height={36} />
              <span style={{ fontSize: 18, fontWeight: 800, letterSpacing: '-0.02em' }}>Cifrinho</span>
            </a>
          </div>
          ...
          <div style={{ display: 'flex', flex: 1, alignItems: 'center', justifyContent: 'center', padding: '32px 0' }}>
            <div style={{ width: '100%', maxWidth: 448 }}>
              <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.02em', margin: 0 }}>{heading}</h1>
              <p style={{ marginTop: 8, fontSize: 16, color: 'rgb(var(--c-mute))' }}>{subtext}</p>

              {/* abas de navegação */}
              <div style={{ marginTop: 28, display: 'flex', gap: 28, borderBottom: '1px solid rgb(var(--c-line))' }}>
                <button className={`auth-tab${tab === 'login' ? ' on' : ''}`} onClick={() => { setTab('login'); setError('') }}>Entrar</button>
                <button className={`auth-tab${tab === 'signup' ? ' on' : ''}`} onClick={() => { setTab('signup'); setError('') }}>Criar conta</button>
              </div>
              ...
              <div style={{ marginTop: 28, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                >
                  <svg style={{ width: 20, height: 20 }} viewBox="0 0 24 24">
                    ...
                    <path fill="#1976D2" d="M43.6 20.5H24v8h11.3c-.8 2.3-2.2 4.2-4 5.6l6.3 5.3C41.4 36.5 44 30.8 44 24c0-1.3-.1-2.3-.4-3.5z"/>
                  </svg>
                  Google
                </button>
              ...
              <div style={{ margin: '24px 0', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 500, color: 'rgb(var(--c-faint))' }}>ou com e-mail</span>
                <span style={{ height: 1, flex: 1, background: 'rgb(var(--c-line))' }} />
              </div>
              ...
              {error && (
                <div style={{
                  ...
                  background: 'rgb(var(--c-rose) / .1)', border: '1px solid rgb(var(--c-rose) / .2)',
                  color: 'rgb(var(--c-rose))', fontSize: 14,
                }}>
                  {error}
                </div>
              )}
                ...
                <form key="login" className="pane-enter" onSubmit={handleLogin}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', pointerEvents: 'none' }}>
                        <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="m3 7 9 6 9-6"/></svg>
                      </span>
                      <input type="email" className="auth-field" placeholder="E-mail" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} />
                    </div>
                    {/* campo de senha */}
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', pointerEvents: 'none' }}>
                        <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>
                      </span>
                      <input
                      ...
                      />
                      <button type="button" onClick={() => setLoginPwVisible(v => !v)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', cursor: 'pointer', background: 'none', border: 0, padding: 0 }}>
                  ...
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: 'rgb(var(--c-mute))', cursor: 'pointer' }}>
                      <button
                        type="button"
                        role="switch"
                    ...
                    <a href="#" style={{ fontSize: 14, fontWeight: 600, color: 'rgb(var(--c-brand))', textDecoration: 'none' }}
                  ...
                  <button type="submit" disabled={loading} style={{
                    marginTop: 24, display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'center', gap: 8,
                    borderRadius: 12, background: 'rgb(var(--c-brand))', padding: '14px', fontSize: 16, fontWeight: 700,
                    color: '#fff', border: 0, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1,
                    boxShadow: '0 4px 20px rgb(var(--c-brand) / .2)', transition: 'background-color .18s ease, box-shadow .18s ease',
                    fontFamily: 'inherit',
                  }}>
              ...
              {tab === 'signup' && (
                <form key="signup" className="pane-enter" onSubmit={handleRegister}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', pointerEvents: 'none' }}>
                        <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 3.6-6 8-6s8 2 8 6"/></svg>
                      </span>
                      <input type="text" className="auth-field" placeholder="Nome completo" required value={signupName} onChange={e => setSignupName(e.target.value)} />
                    </div>
                    {/* campo de e-mail */}
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', pointerEvents: 'none' }}>
                      ...
                      <input type="email" className="auth-field" placeholder="E-mail" required value={signupEmail} onChange={e => setSignupEmail(e.target.value)} />
                    </div>
                    {/* campo de senha */}
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', pointerEvents: 'none' }}>
                      ...
                      <input
                        ...
                        onBlur={() => setPwFocused(false)}
                      />
                      <button type="button" onClick={() => setSignupPwVisible(v => !v)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: 'rgb(var(--c-faint))', cursor: 'pointer', background: 'none', border: 0, padding: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }} aria-hidden="true">
                      {[0, 1, 2, 3].map(i => (
                        <span key={i} className="strength-bar" style={{
                          background: signupPassword && i < pwScore
                    ...
                    <p style={{ marginTop: -4, fontSize: 12, color: 'rgb(var(--c-faint))' }}>
                      {signupPassword ? STRENGTH_LABELS[Math.max(0, pwScore - 1)] : 'Use 8+ caracteres com letras, números e símbolos.'}
                    </p>
                  </div>

                  {/* aceite dos termos */}
                  <label style={{ marginTop: 16, display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 14, color: 'rgb(var(--c-mute))', cursor: 'pointer' }}>
                    <button
                      type="button"
                      role="switch"
                    ...
                    <span>
                      Concordo com os{' '}
                      <a href="/legal" style={{ fontWeight: 600, color: 'rgb(var(--c-brand))', textDecoration: 'none' }}
                        onMouseEnter={e => (e.currentTarget.style.textDecoration = 'underline')}
                        onMouseLeave={e => (e.currentTarget.style.textDecoration = 'none')}>
                        Termos
                      </a>{' '}e a{' '}
                  ...
                  <p style={{ marginTop: 12, textAlign: 'center', fontSize: 12, color: 'rgb(var(--c-faint))' }}>
              ...
              <p style={{ marginTop: 28, textAlign: 'center', fontSize: 14, color: 'rgb(var(--c-mute))' }}>
                {tab === 'login' ? (
                  <>Ainda não tem conta?{' '}
                    <button onClick={() => { setTab('signup'); setError('') }} style={{ fontWeight: 600, color: 'rgb(var(--c-brand))', background: 'none', border: 0, cursor: 'pointer', fontFamily: 'inherit', padding: 0, fontSize: 'inherit' }}>Criar agora</button>
                  </>
                ) : (
                  <>Já tem conta?{' '}
                    <button onClick={() => { setTab('login'); setError('') }} style={{ fontWeight: 600, color: 'rgb(var(--c-brand))', background: 'none', border: 0, cursor: 'pointer', fontFamily: 'inherit', padding: 0, fontSize: 'inherit' }}>Entrar</button>
                  </>
                )}
              </p>
            </div>
          ...
          <p style={{ textAlign: 'center', fontSize: 12, color: 'rgb(var(--c-faint))' }}>
            ...
            <a href="https://www.juninhos.com/" target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none' }}
              onMouseEnter={e => (e.currentTarget.style.textDecoration = 'underline')}
              onMouseLeave={e => (e.currentTarget.style.textDecoration = 'none')}>
              Comunidade Juninhos
            </a>
