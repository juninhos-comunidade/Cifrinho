...
const LEVEL_THRESHOLDS = [
  0, 100, 300, 600, 1000, 1500, 2200, 3000, 4000, 5200, 6500,
];
...
export async function gamificationRoutes(app: FastifyInstance) {
  ...
  app.get("/gamification/progress", authenticate, async (request) => {
    ...
    const xp =
      txCount * XP_RULES.per_transaction + totalBadges * XP_RULES.per_badge;

    const levelInfo = calcLevel(xp);

    const earned = [...myBadgeRecords.map((ub) => ub.badge), ...newlyUnlocked];
